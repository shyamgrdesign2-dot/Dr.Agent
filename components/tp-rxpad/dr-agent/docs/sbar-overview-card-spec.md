...
│
├── B — History (single unified InlineDataRow)
│   ├── Chronic conditions (no key prefix, compound values)
│   ├── Allergies (key: "Allergies")
│   ├── Family Hx (key: "Family Hx")
│   ├── Lifestyle (key: "Lifestyle")
│   └── Current Meds (key: "Current Meds", max 8, shortened)
│
├── A — Today's Vitals (InlineDataRow with flag colors)
├── A — Key Labs (InlineDataRow with ↑/↓ indicators, max 4)
│
├── Last Visit (SectionTag + one-liner)
│
└── R — Recommendations (SectionTag heading + bullet pointers)
...
### B — History (Unified Background Section)

The History section uses a **single `InlineDataRow`** component that contains the most clinically relevant background. This maps to the SBAR "Background" category.

#### Default display (shown in card)

| Sub-category | Data Field | Key Prefix | Display |
|---|---|---|---|
| Chronic Conditions | `chronicConditions` | `"Conditions"` | Compound value: `"Diabetes (1yr, Active), Hypertension (6mo, Active)"` |
| Allergies | `allergies` | `"Allergies"` | Comma-separated: `"Dust, Egg, Prawns"` |
| Current Medications | `activeMeds` | `"Current Meds"` | Shortened to drug name + strength, max 6 |

#### Extendable fields (available in History sidebar, not shown in summary card)

These fields CAN be included in the History section when available but are omitted from the default SBAR summary to keep it concise. Not every patient will have this data.

| Sub-category | Data Field | Key Prefix |
|---|---|---|
| Family History | `familyHistory` | `"Family Hx"` |
| Lifestyle | `lifestyleNotes` | `"Lifestyle"` |
| Surgical History | *(future field)* | `"Surgical Hx"` |
| Additional History | *(future field)* | `"Additional"` |

**Formatting — Color Hierarchy**:

The History section is rendered **manually** (not via `InlineDataRow`) using the shared `formatWithHierarchy()` utility. This is because `InlineDataRow`'s compound value splitting (`splitCompoundValue`) breaks on commas inside parentheses — e.g., `"Diabetes (1yr, Active)"` would be incorrectly split at the inner comma.

Each item is rendered with `formatWithHierarchy()`:
- Text **before** brackets → `font-medium text-tp-slate-700` (dark, primary — condition/item name)
- Text **inside** brackets → `font-normal text-tp-slate-400` (lighter, secondary — duration/status)
- Sub-category labels (`Conditions:`, `Allergies:`, etc.) → `text-tp-slate-400`
- Pipe dividers (`|`) between sub-categories → `text-tp-slate-200` with `mx-[6px]` spacing (pipe character only, never `·`)
- Items within a sub-category separated by commas in `text-tp-slate-400`

Example rendering:
> **History** Conditions: **Diabetes** (1yr, Active), **Hypertension** (6mo, Active) | Allergies: **Dust** (3yr, Active), **Egg** (Childhood) | Current Meds: **Telma 20mg**, **Metsmail 500mg**

**Why unified History**: In clinical practice, "history" encompasses all background information — conditions, allergies, family, lifestyle, medications. Splitting these into separate sections creates visual clutter and breaks the SBAR "Background" mapping. One consolidated section lets the doctor scan all background in a single row.

**Why only 3 sub-categories by default**: Family history, lifestyle, and surgical history are not universally available for all patients. Showing empty or sparse sub-categories clutters the summary. The doctor can access the full history via the History sidebar link (clickable tag).

**Why medications are in History**: Current medications are part of the patient's background state — they inform clinical decisions before assessment. In SBAR, active medications belong in "Background" not "Recommendation".

**Empty state**: Section is hidden entirely if all sub-categories are empty.
...
Simple bullet list under the "Recommendations" SectionTag:
- Bullet: `●` in `tp-slate-300` (subtle)
- Text: `text-tp-slate-600` at `12px`
...
| P2 — Returning, no intake | Situation, History, Vitals, Labs, Last Visit, Recommendations |
...
| History | Unified single row with all background | Separate chronic + allergies rows |
