...
/* ── Convert VitalTrendSeries[] → Recharts row data ── */
...
/* ── Legend row — below the chart, identical for both modes ── */
      ...
      <div style={{ minWidth, height: chartHeight }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 16, right: 8, bottom: 4, left: -8 }} barGap={2} barCategoryGap="20%">
            ...
            {allSeries.map((s, si) =>
              s.threshold != null ? (
                <ReferenceLine key={`thresh-${si}`} y={s.threshold} stroke={SERIES_COLORS[si % SERIES_COLORS.length]} strokeDasharray="4 2" strokeOpacity={0.5} label={{ value: s.thresholdLabel ?? "", position: "right", fontSize: 10, fill: SERIES_COLORS[si % SERIES_COLORS.length] }} />
              ) : null
            )}
            {allSeries.map((s, si) => (
              <Bar key={s.label} dataKey={s.label} fill={SERIES_COLORS[si % SERIES_COLORS.length]} radius={[3, 3, 0, 0]} maxBarSize={24} label={{ position: "top", fontSize: 9, fill: SERIES_COLORS[si % SERIES_COLORS.length], fontWeight: 600 }} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div style={{ minWidth, height: chartHeight }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 16, right: 8, bottom: 4, left: -8 }}>
            ...
            {allSeries.map((s, si) =>
              s.threshold != null ? (
                <ReferenceLine key={`thresh-${si}`} y={s.threshold} stroke={SERIES_COLORS[si % SERIES_COLORS.length]} strokeDasharray="4 2" strokeOpacity={0.5} />
              ) : null
            )}
            {allSeries.map((s, si) => (
              <Line key={s.label} type="monotone" dataKey={s.label} stroke={SERIES_COLORS[si % SERIES_COLORS.length]} strokeWidth={2} dot={{ r: 3, fill: "white", strokeWidth: 1.5 }} activeDot={{ r: 5 }} label={{ position: "top", fontSize: 9, fill: SERIES_COLORS[si % SERIES_COLORS.length], fontWeight: 600 }} />
            ))}
          </LineChart>
        </ResponsiveContainer>
      </div>
