...
export function RevenueBarCard({ data, onPillTap }: Props) {
  ...
  const handleDownload = () => {
    downloadAsExcel(
      isDepositMode ? "deposit_report" : "billing_report",
      ["Day", "Primary (₹)", "Secondary (₹)", "Refunded (₹)", "Total (₹)"],
    ...
    >
      ...
      <div className="mb-[10px] grid grid-cols-2 gap-[6px]">
        ...
        <div className="rounded-[8px] px-[8px] py-[7px]" style={{ backgroundColor: isDepositMode ? "rgba(239,68,68,0.08)" : "rgba(34,197,94,0.08)" }}>
          <span className="block truncate text-[13px] font-semibold leading-tight" style={{ color: isDepositMode ? "var(--tp-danger-600, #DC2626)" : "var(--tp-success-600, #16A34A)" }}>
            &#x20B9;{isDepositMode ? data.totalRefunded.toLocaleString("en-IN") : data.totalPaid.toLocaleString("en-IN")}
          </span>
          <span className="mt-[3px] block text-[9px] font-medium uppercase tracking-wide" style={{ color: isDepositMode ? "var(--tp-danger-500, #EF4444)" : "var(--tp-success-500, #22C55E)" }}>
        ...
        <svg width={svgWidth} height={chartHeight + 20} className="block">
          ...
          {data.days.map((day, i) => {
            ...
            const segments = [
              ...
              { key: "refunded", value: refunded, color: "var(--tp-danger-500, #EF4444)", opacity: 0.9 },
