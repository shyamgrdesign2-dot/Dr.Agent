...
const CARD_SPECS: CardSpec[] = [
  // ── Summary ──
  { kind: "patient_summary", family: "Summary", description: "Patient Overview (SBAR) — the primary patient summary card. Follows SBAR protocol: Situation (narrative), Background (history + allergies), Assessment (vitals + labs), Recommendation (action items). This is the default patient summary shown across all contexts.", intent: "data_retrieval", whenToShow: "'Patient summary', 'overview', 'snapshot', 'Patient's detailed summary' pill. Default card when patient context is opened.", permutations: ["Full SBAR with all sections", "Without labs", "With chronic conditions", "With symptom collector", "With specialty alerts", "With vitals abnormals", "With critical vitals", "With follow-up overdue"], dataParams: "SmartSummaryData", dataSources: "EMR: vitals, labs, conditions, meds, allergies. Uploaded documents (AI-extracted). Symptom collector (patient-reported). Specialty modules (obstetric/gynec/ophthal/pediatric).", formattingNotes: "SBAR section headers (Situation, Background, Assessment, Recommendation) with colored left borders (violet/blue/emerald/amber). Inline data rows: primary in tp-slate-700 (#454551), (details) in tp-slate-400 (#A2A2A8), dividers in tp-slate-200 (#E2E2EA). Provenance dots: EMR tp-success-600, AI tp-warning-600. Narrative in tp-violet-600 italic.", cannedMessages: ["Patient summary", "Overview", "Patient's detailed summary", "Show patient snapshot", "SBAR summary"], intentSummary: "data_retrieval → Card: multi-section SBAR clinical overview too complex for text" },
