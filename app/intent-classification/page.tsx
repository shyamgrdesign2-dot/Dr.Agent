...
/** Column titles for the section-tag education table — same chip spec as SectionTag (30px row, 18px icon, 14px semibold). */
const SECTION_TAG_CATALOG_TABLE_HEADERS: {
  label: string
  icon: string
  variant?: "default" | "specialty"
}[] = [
  { label: "Icon", icon: "health-file-02" },
  { label: "Section tag", icon: "Diagnosis" },
  { label: "Used in", icon: "medical-report" },
  { label: "Variant", icon: SECTION_TAG_ICON_MAP["Obstetric"], variant: "specialty" },
]
...
const ALL_SECTION_TAGS = [
  ...
  { tag: "Symptoms", usedIn: "Reported by Patient, Last Visit, Voice Rx", variant: "default", iconName: SECTION_TAG_ICON_MAP["Symptoms"] },
...
const CARD_SPECS: CardSpec[] = [
  // ── Summary ──
  { kind: "patient_summary", family: "Summary", description: "Patient Overview — the single unified patient summary card following the SBAR protocol. Sections: S (Situation) — AI-generated narrative with specialty lead-in, symptoms, chronic conditions, allergies, meds. B (Background) — medical history, conditions, allergies, surgical history, family history, lifestyle. A (Assessment) — today's vitals with flags + key labs (abnormals only). R (Recommendation) — overdue follow-ups with days count, critical vital alerts (↑↓ arrows in tp-error-600), concerning trends, due investigations, cross-problem flags. Optional: Last Visit summary (date, Dx, Rx) shown below Assessment for continuity of care. Each section is independently visible/hidden based on data availability. Card degrades gracefully from full-data to zero-data.", intent: "data_retrieval", whenToShow: "'Patient summary', 'overview', 'snapshot', 'Patient's detailed summary' pill. Default card when patient context is opened. Also triggered by 'SBAR summary', 'Emergency overview', 'On-call handoff'.", permutations: ["Full SBAR with all sections", "Without labs (no abnormals)", "Without Background (new patient)", "With symptom collector (reported by patient)", "With specialty embed (obstetric/gynec/ophthal/pediatric)", "With critical vitals (BP/SpO2 flagged)", "With follow-up overdue", "Zero data (new patient fallback)"], dataParams: "SmartSummaryData", dataSources: "EMR historical modules: past visits, vitals history, medical history, specialty data (ophthal/gynec/obstetric/pediatric), lab results, uploaded medical records. Current encounter: symptom collector payload, current visit vitals, RxPad entries. Identity: patient demographics, appointment context (new/follow-up), appointment status (queue/finished/cancelled/draft).", formattingNotes: "SBAR sections with colored left borders — S: tp-violet-300, B: tp-blue-300, A: tp-success-300, R: tp-warning-300. Situation narrative in tp-violet-600 italic (150-400 chars). Inline data rows: primary in tp-slate-700 (#454551), (parenthetical details) in tp-slate-400 (#A2A2A8), dividers · | in tp-slate-200 (#E2E2EA). Flag arrows ↑↓ in tp-error-600 (#C8102E). Provenance dots: EMR tp-success-600 (#059669), AI tp-warning-600 (#D97706). Section tags with 12px font-medium. Font: Inter 10-12px body, 9px captions.", cannedMessages: ["Patient summary", "Overview", "Patient's detailed summary", "Show patient snapshot", "SBAR summary", "Emergency overview", "On-call handoff"], intentSummary: "data_retrieval → Card: multi-section SBAR clinical overview with conditional section visibility — too complex for text" },
  { kind: "symptom_collector", family: "Summary", description: "Patient-reported symptoms from pre-visit intake form.", intent: "data_retrieval", whenToShow: "'Reported by patient' pill.", permutations: ["Full intake", "Partial intake", "With severity ratings"], dataParams: "SymptomCollectorData", dataSources: "Patient-reported pre-visit intake form. Symptoms, medical history, family history, allergies, lifestyle, and questions to doctor.", formattingNotes: "Section-tagged rows — symptoms with severity badges, history as tag pills, allergies in red-tinted row. Questionnaire format with expandable sections.", cannedMessages: ["Reported by patient", "Symptoms reported", "What did the patient report?", "Intake form"], intentSummary: "data_retrieval → Card: multi-section intake data with severity ratings needs structured layout" },
...
└─────────────────────────────────────────┘`}</pre>
        ...
        <section>
          ...
          <p className="mb-4 ml-9 text-[11px] text-slate-400">
            {ALL_SECTION_TAGS.length} documented tags across all cards. Always paired with icons — never text-only.{" "}
            <strong className="text-slate-600">Column titles</strong> use production{" "}
            <code className="text-[10px] text-violet-600">SectionTag</code> chips (30px row, 18px icon, 14px semibold).
          </p>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-2">
              <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
                <table className="min-w-full text-[10px]">
                  <thead>
                    <tr className="border-b border-slate-100 bg-[#FAFAFB]">
                      {SECTION_TAG_CATALOG_TABLE_HEADERS.map((h) => (
                        <th key={h.label} className="px-2 py-2 align-bottom text-left font-normal">
                          <SectionTag
                            label={h.label}
                            icon={h.icon}
                  ...
                  <tbody>{ALL_SECTION_TAGS.map(t => (
                    <tr key={t.tag + t.usedIn} className="border-b border-slate-50">
                      <td className="px-2.5 py-1.5 align-middle">
                        <span
                          className={cn(
                            "inline-flex h-[30px] w-[30px] items-center justify-center rounded-[4px]",
                            t.variant === "specialty" ? "bg-tp-violet-50" : "bg-tp-slate-100/70",
                          )}
                        >
                          <TagIconPreview iconName={t.iconName} variant={t.variant} size={18} />
                        </span>
                      </td>
                      <td className="px-2.5 py-1.5 align-middle">
                        <SectionTag label={t.tag} icon={t.iconName} variant={t.variant} className="pointer-events-none" />
                      </td>
                      <td className="px-2.5 py-1.5 align-middle text-[12px] leading-snug text-slate-600">{t.usedIn}</td>
                      <td className="px-2.5 py-1.5 align-middle">
                        <span
                          className={cn(
                            "inline-flex h-[30px] items-center rounded-[4px] px-2 text-[14px] font-semibold leading-none",
                            t.variant === "specialty"
  ...
  const embeddedContent = (
    <div>
      <div className="sticky top-[120px] z-30 -mx-6 px-6 bg-white/95 border-b border-slate-200 backdrop-blur-md">
